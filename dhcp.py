...
def puci_dhcp_static_leases_config_list():
    ...
    for i in range (0, len(name)):
        ...
        rc = puci_dhcp_static_leases_config_retrieve(name[i], 0)
        if i == 0:
            leases_data = [rc]
        else:
            leases_data.append(rc)

    data = {
            "static-lease-list": leases_data,
            'header': {
                        'resultCode': 200,
                        'resultMessage': 'Success.',
                        'isSuccessful': 'true'
...
def puci_dhcp_static_leases_config_retrieve(name, add_header):
    leases_data = dict()
    leases_data = dhcp_static_leases_uci_get(name, leases_data)
...
def puci_dhcp_static_leases_config_create(request):
    return dhcp_static_leases_set(request)

def puci_dhcp_static_leases_config_update(request):
    return dhcp_static_leases_set(request)

def puci_dhcp_static_leases_config_detail_create(request, name):
    return dhcp_static_leases_detail_set(request, name)

def puci_dhcp_static_leases_config_detail_update(request, name):
    return dhcp_static_leases_detail_set(request, name)

def dhcp_static_leases_uci_get(name, leases_data):
    uci_config = ConfigUCI(UCI_DHCP_FILE, UCI_DHCP_STATIC_LEASE_CONFIG, name)
    if uci_config == None:
        raise RespNotFound("UCI Config")
...
def dhcp_static_leases_set(request):
    leases_list = request['static-lease-list']

    while len(leases_list) > 0:
        namedata = leases_list.pop(0)

        name = namedata['name']

        dhcp_static_leases_uci_set(namedata, name)
...
def dhcp_static_leases_detail_set(request, name):

    dhcp_static_leases_uci_set(request, name)
...
def dhcp_static_leases_uci_set(req_data, name):
